...
export class ObjectStorageService {
  ...
  // Gets the upload URL for public files (like profile images)
  async getPublicUploadURL(): Promise<string> {
    ...
    const objectId = randomUUID();
    const fullPath = `${publicPath}/profile-images/${objectId}`;
  ...
  // Gets a signed read URL for public files (like profile images)
  async getSignedReadURL(objectPath: string): Promise<string> {
    const { bucketName, objectName } = parseObjectPath(objectPath);

    // Sign URL for GET method with longer TTL (24 hours)
    return signObjectURL({
      bucketName,
      objectName,
      method: "GET",
